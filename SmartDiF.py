...
def start(entrypoint, port):
    activate_this = '../myproject/myprojectenv/bin/activate_this.py'
    exec (open(activate_this).read(), dict(__file__=activate_this))
    browser = Th(entrypoint, port)
    browser.start()
    call('gunicorn --workers=1 --threads=20 --max-requests=5 --timeout=30 --bind ' + entrypoint + ":" + str(
...
def fill_mappings():
    ...
    for f in file_list:
        with open(f) as data_file:
            ...
            mappings[data['name']] = {'url': data['mapping_url'], 'endpoint': data['endpoint'],
                                      'image': data['image_name'], 'virtualization': data['virtualization']}
    return mappings
