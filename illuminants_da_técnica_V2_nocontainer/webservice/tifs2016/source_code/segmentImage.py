...
def segmentImage(fileName,illuminantType):
    print("\nCHEGOU AQUI:segmentImage()")
    nameFaces = "tifs2016/face-positions/" + fileName[:-4] + ".txt"
    ...
    for i in lines:
        ...
        if (len(temp) != 1):
            ...
            pathImage = "tifs2016/data-base/" + illuminantType + "/" + fileName[:-4] + "_fhs.png"
            im = cv2.imread(pathImage)
            region = im[toplefty:bottomrighty,topleftx:bottomrightx]
            nameFace = "tifs2016/data-base/faces/face-" + str(cont) + ".png"
