...
def extractAllFeatureVectors(descriptor,space,channel):
    path = "tifs2016/data-base/images/"
    dirList = os.listdir(path)
    dirList.sort()
    nameSpace,nameChannel = sc.getSpaceChannelName(space,channel)
    tt = descriptor.upper()
    npath = "tifs2016/extracted-feature-vectors/" + tt + "-" + "IIC-" + nameSpace + "-" + nameChannel
    ...
    for fname in dirList:
        try:
            ...
            name = "tifs2016/temp/vectors/fv-" + img[:-3] + "txt"
            command = "cp " + name + " " + npath + "/"
            os.system(command)
            command = "rm tifs2016/data-base/faces/*.ppm"
            os.system(command)
            command = "rm tifs2016/temp/faces/*"
            os.system(command)
            command = "rm tifs2016/temp/vectors/*"
            os.system(command)
        except:
    ...
    npath = "tifs2016/extracted-feature-vectors/" + tt + "-" + "GGE-" + nameSpace + "-" + nameChannel
    if not os.path.exists(npath):
        os.makedirs(npath)
