...
def svmTestBySample(dataset,descriptor,space,channel,illuminant="IIC", testFolds=[5]):
    ...
    for i in testFolds:
        fd = fd+str(i)+"-"
    fd = fd[:-1]
    outfile = "tifs2016/training-test-files/" + tt + "-" + illuminant + "-" + nameSpace + "-" + nameChannel + "/" + dataset + "-SVM-test-folds-" + fd
...
def svmTestByImage(dataset,descriptor,space,channel,illuminant="IIC", testFolds=[5]):
    ...
    for i in testFolds:
        fd = fd+str(i)+"-"
    fd = fd[:-1]
    outfile = "tifs2016/training-test-files/" + tt + "-" + illuminant + "-" + nameSpace + "-" + nameChannel + "/" + dataset + "-test-folds-" + fd
